...
function parseProxyString(proxyStr) {
    ...
    if (str.startsWith('[')) {
        // Bracketed IPv6 address: [2001:db8::1]:1080:user:pass
        ...
        if (parts.length === 4) {
            ...
            // Raw unbracketed IPv6 with user:pass or trailing port
...
function createBot(username, host, port, password, proxyUrl, reconnectDelay, socket) {
    ...
    // Low-level TCP tunnel directly through SOCKS5 stream
    if (parsedProxy) {
